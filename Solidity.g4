...
inheritanceSpecifier
  : userDefinedTypeName ( callArgumentList )? ;
...
stateVariableDeclaration
  : typeName
    ( PublicKeyword | InternalKeyword | PrivateKeyword | ConstantKeyword| OverrideKeyword| ImmutableKeyword )*
...
functionDefinition
  : natSpec? ('function'|'receive'|'fallback') identifier? parameterList  modifierList returnParameters?  ( ';' | block ) ;

returnParameters
  : 'returns' parameterList ;

overrideParameters
   : 'override' parameterList ;


modifierList
  : ( modifierInvocation | stateMutability | ExternalKeyword
    | PublicKeyword | InternalKeyword | PrivateKeyword| OverrideKeyword | overrideParameters | VirtualKeyword)* ;
...
catchClause
: 'catch' ((identifier)? '(' parameterList natSpec?')')? block
| 'catch' block;
...
ifStatement
  : 'if' '(' ifCondition ')' ifRight ( 'else' ifWrong )? ;
...
expression
  ...
  |expression '{'(identifier ':' expression (',' identifier ':' expression)*)? '}'
...
DecimalNumber
  : ( DecimalDigits | (DecimalDigits? '.' DecimalDigits) ) ( [eE] ('_')? DecimalDigits )? ;
